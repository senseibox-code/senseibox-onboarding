...
APP_NAME="senseibox-onboarding"
SERVICE_NAME="senseibox-onboarding"
SERVICE_USER="senseibox"
SERVICE_GROUP="senseibox"
INSTALL_ROOT="/opt/senseibox"
INSTALL_DIR="${INSTALL_ROOT}/${APP_NAME}"
STATE_DIR="/var/lib/senseibox/onboarding"
LOG_DIR="/var/log/senseibox"
...
install -d -o "${SERVICE_USER}" -g "${SERVICE_GROUP}" "${STATE_DIR}"
install -d -o "${SERVICE_USER}" -g "${SERVICE_GROUP}" "${LOG_DIR}"
...
install -m 0755 "${INSTALL_DIR}/bin/senseibox-setup" /usr/bin/senseibox-setup
install -m 0755 "${INSTALL_DIR}/bin/senseibox-wifi-setup" /usr/bin/senseibox-wifi-setup
